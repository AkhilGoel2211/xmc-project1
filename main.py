...
from utils import calculate_head_tail_metrics  # Importing the new head-tail metrics function
...
def main(cfg: SimpleConfig) -> None:
    ...
    # Dynamically modify the config file
    cfg2 = cfg.dataset
    with open_dict(cfg2):
        cfg2['environment'] = cfg['environment']
    cfg = cfg2

    # Initialize data handler and datasets
    ...
    # Head-Tail Label Extraction
    head_labels = data_handler.get_head_labels()
    tail_labels = data_handler.get_tail_labels()
    ...
    # Calculate Head-Tail Metrics Post Training
    print("\nCalculating Head-Tail Metrics...")
    metrics = calculate_head_tail_metrics(cfg, test_loader, runner.model, head_labels, tail_labels)
    print(f"Head Metrics: P@1: {metrics['head']['P@1']:.4f}, P@3: {metrics['head']['P@3']:.4f}, P@5: {metrics['head']['P@5']:.4f}")
    print(f"Tail Metrics: P@1: {metrics['tail']['P@1']:.4f}, P@3: {metrics['tail']['P@3']:.4f}, P@5: {metrics['tail']['P@5']:.4f}")
