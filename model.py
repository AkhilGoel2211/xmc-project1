...
class SimpleTModel(nn.Module):
    '''
     Updated Transformer model supporting head-tail label distinction.
    '''
    ...
    def configure_components(self, cfg):
        """ Configure components including dropout, linear layers, and head-tail layers. """
        ...
        # Head and Tail Layer Distinction
        self.num_head_labels = cfg.data.num_head_labels
        self.num_tail_labels = cfg.data.num_tail_labels
        self.tail_embedding_dim = cfg.model.tail_embedding_dim

        # Head: Dense layer for head labels
        self.head_dense = nn.Linear(cfg.model.ffi.input_features, self.num_head_labels).to(self.device)

        # Tail: Sparse layer for tail labels using EmbeddingBag
        self.tail_sparse = nn.EmbeddingBag(
            num_embeddings=self.num_tail_labels,
            embedding_dim=self.tail_embedding_dim,
            mode="mean",
            sparse=True
        ).to(self.device)

        print(f"Head labels: {self.num_head_labels}, Tail labels: {self.num_tail_labels}")
    ...
    def forward(self, tokens, masks, tail_indices):
        ''' Forward pass through the model with head-tail distinction. '''
        ...
        # Head and Tail Forward Pass
        # Head labels through dense layer
        head_logits = self.head_dense(out)

        # Tail labels through sparse embedding
        tail_logits = self.tail_sparse(tail_indices)

        return head_logits, tail_logits, branch_out  
    ...
    def param_list(self):
        ...
        param_list_xmc += [
            {"params": self.head_dense.parameters(), "lr": self.cfg.training.optimization.lr, 'weight_decay': self.cfg.training.optimization.wd},
            {"params": self.tail_sparse.parameters(), "lr": self.cfg.training.optimization.lr, 'weight_decay': self.cfg.training.optimization.wd}
        ]
